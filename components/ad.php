<div class="vertical-content-block">
    <div class="vertical-cms-slider w-slider">
        <div class="vertical-header-contain">
            <h2 class="vertical-header">Weblionz Agency</h2>
        </div>
        <div class="w-slider-mask">
            <div class="w-slide">
                <div class="vertical_slider_collection_list_wrapper w-dyn-list">
                    <div role="list" class="vertical-slider-collection-list w-dyn-items">
                        <div role="listitem" class="vertical-slider-collection-item w-dyn-item">
                            <a target="_blank" href="https://weblionz.com/"
                                class="vertical-slide-content-contain w-inline-block">
                                <div class="vertical-slide-content">
                                    <h4 class="white">
                                    Creative web Agency - Create your website with us.
                                    </h4>
                                </div>
                                <div class="overlay"></div>
                                <div style="background-color:#FDA310" class="sticker on-article-card rtl">
                                    <div style="color:white">
                                    حملة إشهارية
                                    </div>
                                </div>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
      
        </div>
    </div>
</div>